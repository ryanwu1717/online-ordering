...
const Graph = (props) => {
        ...
        axios
            ...
            .then((response) => {
                ...
                response["data"].map((value) => {
                    return (setProcesses_name_show(old => [...old, value["製程名稱"]]),
                        setProcesses_id_show(old => [...old, value["製程代號"]]),
                ...
                response["data"].map((value) => {
                    return (setProcesses_name_show_finish(old => [...old, value["製程名稱"]]),
                        setProcesses_id_show_finish(old => [...old, value["製程代號"]]),
    ...
    const DayChange = () => {
        setStartDate(moment(StartDateTime.current.value).format('YYYY-MM-DD HH:mm:ss'))
        // console.log(EndDateTime.current.value)
        if (EndDateTime.current.value.length != 0) {
            setEndDate(moment(EndDateTime.current.value).format('YYYY-MM-DD HH:mm:ss'))
        }
    }
    useEffect(() => {
        refreshBarChart()
                ...
                <Col md='12'>
                    <Card className='shadow mb-4 w-100'>
                        <Card.Title md='12' as="h3" className='position-relative mb-5'>
                            <Rfid_title className="badge rounded position-absolute rfid_title p-3 text-center top-0 start-0 ">廠內製程數量趨勢圖</Rfid_title>
                        </Card.Title>
