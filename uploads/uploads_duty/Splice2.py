...
dim = (int(d_x/4), int(d_y/4))
im_N = cv2.resize(im_N, dim, interpolation = cv2.INTER_AREA)
d_x = im_N.shape[1]
d_y = im_N.shape[0]
...
im_b = im_N[:,:,2]
if(d_y > d_x):
	im_r = np.rot90(im_r,1)
	im_g = np.rot90(im_g,1)
	im_b = np.rot90(im_b,1)
	tmp = d_x
	d_x = d_y
	d_y = tmp
...
#outputImg[topLeft[0],topLeft[1],0] = 0
#outputImg[topLeft[0],topLeft[1],1] = 255
#outputImg[topLeft[0],topLeft[1],2] = 255
...
nextPos = moveVer(new_r,new_g,new_b,topLeft,1,1)
nextPos = moveHor(new_r,new_g,new_b,nextPos,1,1)
nextPos = moveVer(new_r,new_g,new_b,nextPos,-1,1)
print(nextPos)
BPTopLeft = nextPos
nextPos = moveVer(new_r,new_g,new_b,nextPos,1,2)
nextPos = moveHor(new_r,new_g,new_b,nextPos,1,5)
print(nextPos)
BPBotRight = nextPos
...
#print(getDensity(BPImg,BPTopLeft,BPBotRight))
...
cv2.imwrite('BP_BK.png', BPImg)
BPCanvas = np.full([BP_d_y,BP_d_x],255)
#getConnectedShape_Opti(BPImg,BPCanvas,120,120,2,-1,-1)
...
for i in range(BP_d_x):
	for j in range(BP_d_y):
		if BPImg[j,i,1] < 210:
			...
			if count > 1000:
				cv2.imwrite('shapes/ShapeAt('+str(i)+','+str(j)+').png', BPCanvas)
